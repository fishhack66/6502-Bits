; *************************************************************
; * SUPERMON GS6 based on JIM BUTTERFIELD's SUPERMON+ 64 V1.2 *
; *************************************************************

; Originally for the Commodore PET, 1980, with the following credits:
;    Dissassembler by Steve Wozniak and Allen Baum
;    Single step by Jim Russo
;    Most other stuff (,HAFT) by Bill Seiler
;    Tidied & Wrapped by Jim Butterfield

; This version adapted from J.B. Langston's reformatted and annotated
; SUPERMON+ 64 source (awesome work, JBL!):
; https://github.com/jblang/supermon64/blob/master/supermon64.asm
; by Andrew Taylor (https://github.com/ataylor284) - also awesome!
; Readapted for 64TASS and enhanced for the GS6 homemade 6502 computer
; with eWoz monitor by Dave Hassler (https://triplecharlie.org)
; 
...
; the KIM-1 itself.  I've (A.T.) taken the source and ported it back
; to the KIM once more.
;
...
; *** FOR GS6: assemble with 64TASS:
;	64tass --s-record supermon-gs6.asm -o supermon-gs6.srec
;	-m -L supermon-gs6.lst
...
; -----------------------------------------------------------------------------
; eWOZ variables
NMIVEC  = $FE1B             ; NMI vector (triggered by ST key on KIM)
BKVEC   = $FE18             ; BRK instruction vector (shared with IRQ)

; -----------------------------------------------------------------------------
; variables  - ***Will go to Zero Page
;.bss
...
; -----------------------------------------------------------------------------
; eWOZ entry points (GS6)
GETCH   = $FFB8             ; input a character (GETCHAR)
OUTCH   = $FF27             ; output a character (ECHO)

; -----------------------------------------------------------------------------
; set up origin for ROM, but could go anywhere (I made a RAM version, too) -DHH
...
SUPER   LDY #MSG4-MSGBAS    ; display banner
        ...
        STA NMIVEC+1
        STA BKVEC+1
;        BRK			; don't do this for GS6, just fall through
...
; -----------------------------------------------------------------------------
; exit monitor [X]
EXIT    JMP $FE00           ; jump to GS6's eWOZ monitor (RESET)
...
DSPM11  LDA #$0F            ; if not, show 16 (12) lines by default
        STA TMP0
...
DSMNEW  JSR SUB12           ; end addr given, calc bytes between start and end
        ...
        LDX DISPMEM_BPL_LOG2; divide by 16 [was '8 (shift right 3 times)']
DSPM01  LSR TMP0+1
        ROR TMP0
        DEX 
        BNE DSPM01
...
ALTMX   ;LDA #$91           ; move cursor up [no screen editor]
        ;JSR CHROUT
...
DMEMGO  LDA (TMP2),Y        ; load byte from start address + Y
        ...
        INY                 ; next byte
        CPY DISPMEM_BPL     ; have we output 16 (8) bytes yet?
...
; -----------------------------------------------------------------------------
; AUTO ADVANCE ADDRESS FOR TTY OPERATION. The C64 version has code here
; for the screen editor and input buffer, which Andrew took out for his KIM-1
; version.  All I did was re-write the buffer code to pre-stuff it with 'A'
; and the next address and add it here, down to CHEK2B.  -DHH
...
DOADDR	JSR ASCTWO	; CONVERT BYTE IN AC INTO HEX DIGITS
...
MSG4    .BYTE $0D, $0D
	.text "SUPERMON GS6"    ; SUPERMON KIM (GS6) banner (or whatever! ;^)
        .BYTE $8D,$8A
...
; [*** Not implimented for GS6  -DHH]
K_SAVED_Y: .byte 0

; -----------------------------------------------------------------------------
; KIM LD - BOTH UNAPPLICABLE FOR GS6
KIM_LD  JMP ERROR
...
; [***While not necessary to use the routines in eWoz Mon, it doesn't hurt
;     anything to save the YR, push/pull the ACC, etc.  -DHH]
