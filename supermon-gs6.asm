...
; SUPERMON+ 64 source by Andrew Taylor (https://github.com/ataylor284)
; Adapted again for the GS6 homemade 6502 computer with eWoz monitor
; by Dave Hassler (https://triplecharlie.org)
; 
...
; -----------------------------------------------------------------------------
; setup ca65 for the supermon source dialect  **DON'T USE with 64TASS

;.feature labels_without_colons, loose_char_term, pc_assignment, 
...
IRQADR	=	$F1	; 2 bytes for user IRQ routine
			; This MUST be your system monitor's BRK
			; vector in RAM. See SUPER label below.
			; As for the GS6's version of eWoz, the hardware
			; IRQ/BRK at $FFFE points to a routine that checks
			; this location for a custom handler.  -DHH
...
SUPER   LDY #MSG4-MSGBAS    ; display banner
        JSR SNDMSG
        LDA #16			; originally 8 for a 40-col C64 screen...
        STA DISPMEM_BPL 
        LDA #4			; ...and a 3 here, to match.
        STA DISPMEM_BPL_LOG2
        LDA LINKAD          ; set BRK vectors
;        STA NMIVEC
        STA IRQADR
        LDA LINKAD+1
;        STA NMIVEC+1
        STA IRQADR+1
;        BRK		; just fall through for GS6 version
...
MSG4    .BYTE $0D, $0D
	.text "SUPERMON GS6" ; SUPERMON KIM (GS6) banner
        .BYTE $8D,$8A
...
SUPAD   .WORD SUPER             ; address of entry point  UNUSED in GS6 ver.
